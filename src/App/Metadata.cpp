...
Metadata::Metadata(const fs::path& metadataFile)
{
    ...
    case 1:
        ...
        break;
    default:
        throw Base::XMLBaseException("pacakge.xml format version is not supported by this version of FreeCAD");
    }
}
...
void Metadata::appendToElement(DOMElement* root) const
{
    ...
    if (_freecadmin != Meta::Version())
        auto element = appendSimpleXMLNode(root, "freecadmin", _freecadmin.str());

    if (_freecadmax != Meta::Version())
        auto element = appendSimpleXMLNode(root, "freecadmax", _freecadmin.str());
...
{
    return std::tie(major, minor, patch, suffix) < std::tie(major, minor, patch, suffix);
}
